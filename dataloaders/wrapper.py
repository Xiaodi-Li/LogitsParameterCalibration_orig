...
class AppendName(data.Dataset):
    ...
    """
    def __init__(self, dataset, name, first_class_ind=0, is_transformer=False):
    ...
    def __getitem__(self, index):
        ...
        if self.is_transformer:
            sample, mask, type_ids, target = self.dataset[index]
            input = [sample, mask, type_ids]
            #input = {"input_ids": sample, "attention_mask": mask}
            target = target + self.first_class_ind
            return input, target, self.name
        else:
            img,target = self.dataset[index]
            target = target + self.first_class_ind
