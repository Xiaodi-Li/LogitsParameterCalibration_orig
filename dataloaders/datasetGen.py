...
def PermutedGen(train_dataset, val_dataset, n_permute, remap_class=False, is_transformer=False):
    if is_transformer:
        sample, _, _, _ = train_dataset[0]
    else:
    ...
    for i in range(1,n_permute+1):
        ...
        if i==1:  # First task has no permutation
            train_datasets[name] = AppendName(train_dataset, name, is_transformer=is_transformer)
            val_datasets[name] = AppendName(val_dataset, name, is_transformer=is_transformer)
        else:
            ...
            train_datasets[name] = AppendName(Permutation(train_dataset, rand_ind), name, first_class_ind=first_class_ind, is_transformer=is_transformer)
            val_datasets[name] = AppendName(Permutation(val_dataset, rand_ind), name, first_class_ind=first_class_ind, is_transformer=is_transformer)
        if is_transformer:
            task_output_space[name] = torch.unique(train_dataset.tensors[3]).shape
        else:
