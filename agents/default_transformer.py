...
class NormalNN(nn.Module):
    ...
    def update_model(self, inputs, tasks=['1']):
        out = self.forward(inputs)
        loss = self.criterion(out, tasks)
    ...
    def learn_batch(self, train_loader, val_loader=None):
        ...
        for epoch in range(self.config['schedule'][-1]):
            ...
            global_step = int(self.config.model_name_or_path.split("-")[-1].split("/")[0]) if (
                    'checkpoint-' in self.config.model_name_or_path) else 0
            epochs_trained = global_step // (len(train_loader) // self.config.gradient_accumulation_steps)
            steps_trained_in_current_epoch = global_step % (len(train_loader) // self.config.gradient_accumulation_steps)
